...
(async () => {
  ...
  const res = await axios.get('https://raw.githubusercontent.com/OloloPhilolo/wordle-by/master/wordslists.js');
  const script = esprima.parseScript(res.data);
  const possibleTasks = _.shuffle(script.body.find(node => node.type === 'VariableDeclaration' && node.declarations[0].id.name === 'possibleTasks').declarations[0].init.elements.map(node => node.value));
// const allowedGuesses = script.body.find(node => node.type === 'VariableDeclaration' && node.declarations[0].id.name === 'allowedGuesses').declarations[0].init.elements.map(node => node.value);
  ...
  */
  function findNextWord(level = 0) {
    if (level === 0) {
      const result = possibleTasks.find(word => {
        if ([...letters.black, ...letters.yellow, ...letters.green].map(letter => word.includes(letter)).includes(true)) return false;
        return _.isEqual(word, _.uniq(word.split('')).join(''));
      });
      if (!result) return findNextWord(level + 1);
      return [result, level];
    }
    if (level === 1) {
      const result = possibleTasks.find(word => ![...letters.black, ...letters.yellow, ...letters.green].map(letter => word.includes(letter)).includes(true));
      if (!result) return findNextWord(level + 1);
      return [result, level];
    }
    if (level === 2) {
      ...
      while (!result && maxLettersCount <= 5) {
        result = possibleTasks.find(word => {
  ...
  const getPossibleAnswers = () => possibleTasks.filter(word => {
    let isValid = true;
